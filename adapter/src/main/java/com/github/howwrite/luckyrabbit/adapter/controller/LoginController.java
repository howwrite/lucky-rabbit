...
public class LoginController {
    ...
    public Response<Boolean> loginBySmsCode(HttpServletRequest httpRequest, HttpServletResponse response, @RequestBody LoginBySmsCodeRequest request) {
        ...
        RequestUtils.writeCookie(response, loginSdkProperties.getLoginCookieName(), loginResult.getLoginToken(), loginResult.getHourToLive());
        return Response.ok(true);
    }

    @GetMapping("/current")
    public Response<Boolean> current() {
        return Response.ok(UserContext.currentUserId() != null);
    }
}
